...
def test_order_total():
    order = Order(("Apple", 1.0), ("Banana", -2.0))
    total = order.total
    assert total == 3.0
...
# %%
def test_order_repr():
    assert inspect.getsource(Order.__repr__) == source
...
def test_order_constructor():
    items = [("Apple", 1.0), ("Banana", -2.0)]
    order = Order(*items)
    assert order._items == [Item(name, abs(price)) for (name, price) in items]


# %%
test_order_constructor()


# %%
def test_order_items():
    items = [("Apple", 1.0), ("Banana", -2.0)]
    expected = [(name, abs(price)) for (name, price) in items]
    order = Order(*items)
