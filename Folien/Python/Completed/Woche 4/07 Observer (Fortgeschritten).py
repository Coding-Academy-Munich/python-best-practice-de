...
class PullProducer:
    def __init__(self):
        self.observers: list[PullItemObserver] = []
        self.last_item: Item | None = None

    def attach(self, observer: PullItemObserver):
        self.observers.append(observer)

    def detach(self, observer: PullItemObserver):
        self.observers.remove(observer)

    def get_last_item(self) -> Item | None:
    ...
    def produce_item(self, name: str, serial_number: int):
        self.last_item = Item(name, serial_number)
        self._notify_observers()


# %%
class PullPrintingObserver(PullItemObserver):
    def __init__(self, producer: PullProducer):
        self.producer = producer

    def update(self):
        item = self.producer.get_last_item()
        if item is not None:
            print(f"Produced: {item.name} (#{item.serial_number})")
